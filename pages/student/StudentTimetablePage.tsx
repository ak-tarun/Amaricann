...
const StudentTimetablePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTimetable = async () => {
      ...
      const [timetableRes, coursesRes] = await Promise.all([
        studentService.getStudentTimetable(),
        courseService.getStudentEnrollments(), // Get only enrolled courses for context
      ]);
      ...
      if (coursesRes.success && coursesRes.data) {
        ...
        console.error(coursesRes.message || 'Failed to fetch enrolled courses for timetable mapping.');
      }
  ...
  sortedTimetable.forEach(entry => {
    // Only show timetable entries for courses the student is actually enrolled in
    if (courses.some(c => c.id === entry.course_id) && timetableByDay[entry.day_of_week]) {
  ...
  return (
    <div className="container mx-auto p-6 min-h-screen-minus-nav-footer">
      <h1 className="text-4xl font-bold text-gray-900 mb-8">My Timetable</h1>
      <p className="text-center text-lg text-gray-700 mb-10">
        Here is your personalized class schedule based on your enrollments.
      </p>

      {timetable.length === 0 ? (
        <p className="text-center text-gray-600 text-lg">No timetable entries found for your enrolled courses.</p>
      ) : (
